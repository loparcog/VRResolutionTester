...
public class LineManager : MonoBehaviour
{
    ...
    {
        if (selectedIdx == scalingFactors.Length - 1) return;
        // Set the current LP to stop being highlighted
        lineGroupDict[scalingFactors[selectedIdx]].GetComponent<LinePairManager>().clearLines();
    ...
    void highlightLP()
    {
        ...
        lineGroupDict[scalingFactors[selectedIdx]].GetComponent<LinePairManager>().highlightLines();
        // Update the text to reflect this change (TODO)
        lineSizeText.text = string.Format("Selected: {0:0.00}mm", scalingFactors[selectedIdx] * 5);
    }
}
